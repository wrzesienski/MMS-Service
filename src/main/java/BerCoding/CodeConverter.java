...
public class CodeConverter {
    ...
    public static String s_dataToHex(ArrayList<String> lisr){
        ...
        if(lisr.isEmpty()){
            ...
            ret = CodeConverter.convertIntToHex(Integer.toString(ret.split(" ").length))
                    + " " + ret;
            ret = CodeConverter.stickId(0, 0,99) + " " + ret;
        }else {
            for (int i = 0; i< lisr.size(); i+=2) {
                ret = CodeConverter.convertStringToHex(lisr.get(i)) + " " + ret;
                ret = CodeConverter.convertIntToHex(Integer.toString(ret.split(" ").length))
                        + " " + ret;
                ret = CodeConverter.stickId(0, 0, Integer.parseInt(lisr.get(i+1))) + " " + ret;
