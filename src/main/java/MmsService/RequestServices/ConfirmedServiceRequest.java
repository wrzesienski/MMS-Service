...
public class ConfirmedServiceRequest implements MyInterface {
    ...
    public Object get(MyBerNode berNode){
        switch (berNode.getId().getTag()){
            case 0: // StatusRequest
                return 0; // here should be bool
            case 1:
                return new GetNameListRequest().get((MyBerNode) berNode.getContent());
            case 2: // IdentifyRequest
                return 0; // here should be bool
            case 3:
                return new RenameRequest().get((MyBerNode) berNode.getContent());
            case 4:
                return new ReadRequest().get((MyBerNode) berNode.getContent());
            case 5:
                return new WriteRequest().get((MyBerNode) berNode.getContent());
